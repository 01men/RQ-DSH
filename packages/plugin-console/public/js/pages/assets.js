      ...
      <div class="asset-card" ${item.type === 'nas' ? `data-nas-id="${esc(item.id)}" title="打开 NAS 详情"` : ''} role="listitem">
        <div class="flex" style="gap:12px;align-items:center">
          ...
          <div class="grow" style="min-width:0">
            ...
            <div class="fs-11 text-4 ellipsis" style="margin-top:3px">${esc(item.org)} · ${esc(item.owner || '—')} · ${item.calls} 次 / ${esc(days)} 天 · ${item.costCents > 0 ? fmtCents(item.costCents) : '无成本'}</div>
          </div>
