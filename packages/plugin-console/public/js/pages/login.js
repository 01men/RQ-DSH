...
import { resolveLanding, LANDING_PREF_KEY } from '../landing.js'
...
export function renderLogin(app) {
  ...
  // 键 heng_ops_next）。读取即消费：参数与暂存只生效一次；同源仅绝对路径（sanitizeNext），
  // 跨源仅回环/私网 http(s)（sanitizeCrossOriginNext，G1），防 open redirect / 票据外泄。
  // 钉钉整页授权离开本页前，跨源目的地暂存 sessionStorage（heng_ops_next_cross），由本页回装
  // 或 SSO 回调脚本消费。
  // next 解析/白名单/出口已提升为共享模块（next-redirect.js，F 清单 P 节）——app.js 已登录态
  // 启动链同规则消费；一次读取 + URL 清参后置由共享面保证（L 节教训不再重演）。
  const { sameOrigin: nextDestination, crossOrigin: crossNextDestination } = consumeNextSources()
  ...
  const finishLogin = async (welcomeName) => {
    ...
    // 跨源：签一次性自助票（TTL ≤120s）随 fragment 回跳，签票失败仍回跳（dsh 侧走
    // 「我已完成扫码」回导校验诚实降级）；同源：直接跳转——规则在 next-redirect.js 共享面
    if (await exitWithNext({ sameOrigin: nextDestination, crossOrigin: crossNextDestination, api })) return
