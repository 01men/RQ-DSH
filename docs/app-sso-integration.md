...
> 只要组织架构功能（组织树/部门成员/负责人）而不需要 email 的应用，用默认机器凭证自助通道
> `GET /api/apps/<appId>/org-sync` 即可（无需管理员追加 scope，支持 `?ifNoneMatch=` 零载荷变更轮询），
> 见 `docs/app-org-sync.md`；本节名册面向需要 email 等更全字段的场景。
